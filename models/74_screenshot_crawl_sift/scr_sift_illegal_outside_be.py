...
def model(dbt, _session):
    ...
    illegal_outside_be = illegal_outside_be.project(
        "sample_id, sift_keypoint_descriptors"
    ).df()

    y_pred = predict_from_descriptors(
        illegal_outside_be["sift_keypoint_descriptors"], sift_clusters
    )
